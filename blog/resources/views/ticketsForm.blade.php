...
<body>
<form method="post" action="/storeTickets/{{$concert_id}}">
    @csrf
    <h1 class=" main-title " >tickets information </h1>
    ...
    <div class="form-group">
        {{--<label for="date">Concert Location</label>--}}
        <input type="text" class="form-control"  name="name" placeholder="ticketName{eg:vip,..,etc} ">
    ...
    {{--<div class="form-group">--}}
        {{--<input type="text" class="form-control"  name="concert_id" placeholder="numberOfTickets"value="{{$concert_id}}" readonly>--}}
        {{--<p class="help is-danger">{{$errors->first('numberOfTickets')}}</p>--}}
    {{--</div>--}}
    <div class="form-group form-check">
        <select class="form-control" name="addAntherTickets">
            <option name="1">I would like to add another type</option>
            <option name="2">I don't want to add another type</option>
        </select>
    </div>
    <br>
    <button type="submit" class="btn btn-primary fa-align-center">set ticket information </button>
</form>
</body>
</html>
